...
<@m.htmlTemplate title="原件已入库" javascripts=[] >
	<!-- BODY BEGIN -->
	<@m.navTips content="工作台 &gt; 提单系统 &gt; 业务处理 &gt; 原件已入库"/>
	...
	<div id="area-form" style="width: 100%;">
		<div style="padding: 10px 60px 15px 5px;">
			<form id="searchForm" class="easyui-form" method="post" data-options="novalidate:true">
				<table cellpadding="5">
					<tr>
						<@m.select selectId="businessCategory" optionId="businessCategoryItem" title="业务类型" onclick="getCodeLibrary1('businessCategory','BusinessCategory');" width="120"/>
	...
	<script type="text/javascript">
        ...
        function getCodeLibrary1(domId,codeNo,itemNo,extendAttribute){
            var optionId = $("#"+domId);
            var selectId = optionId.parent();
            var selectSize= selectId.find("option").length;
            //若调过一次，后面就取缓存
                ...
                $.ajax({
                    ...
                    url : "/bcs-web/codelibrary/getCodeLibrary",
                    data :JSON.stringify({
                        codeNo:codeNo,
                        itemNo:itemNo,
                    ...
                    success : function(result) {
                        ...
                        $.each(cityJson,
                                function(i, val) {
                                    str += '<option value="' + val.itemNo + '">' + val.itemName + '</option>';
                                });
                        //添加到下拉列表
                        optionId.nextAll().remove();//清空
                        optionId.after(str);
                    },
                    error : function() {
                        alert("系统忙，请稍后再试！");
