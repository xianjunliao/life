...
<@m.htmlTemplate title="销售管理" javascripts=[] >
	<!-- BODY BEGIN -->
	<@m.navTips content="工作台 &gt; 提单系统 &gt; 销售管理"/>
	...
	<div id="area-form" style="width: 100%;">
			...
			<form id="searchForm" class="easyui-form" method="post" data-options="novalidate:true">
				<table cellpadding="5">
                    <tr>
						...
						<@m.select selectId="spProvince" optionId="spProvinceItem" title="省" onchange="getCity('spProvince','spCityItem');" width="120"/>
						<@m.select selectId="spCity" optionId="spCityItem" title="市" onchange="getDistrict('spCity','spDistrictItem');" width="120"/>
						<@m.select selectId="spDistrict" optionId="spDistrictItem" title="区域" width="120"/>
					</tr>
					<tr>
						<@m.text id="drmName" title="DRM" width="120"/>
						<@m.select selectId="serviceProvidersType" optionId="serviceProvidersTypeItem" title="经销商类型" onclick="getCodeLibrary('serviceProvidersTypeItem','ServiceProvidersType');" width="120"/>
						<@m.text id="dealerName" title="经销商名称" width="120"/>
                    </tr>
                    <tr>
		...
		<div id="func_operate">
            ...
            <a href="javascript:void(0)" class="easyui-linkbutton" onclick="toViewApprove()">查看审批意见</a>
            <a href="javascript:void(0)" class="easyui-linkbutton" onclick="toViewLoan()">查看放款意见</a>
	...
	<div id="area-dataGrid" style="width:100%;">
		<table id="todoGrid" data-options="toolbar:'#area-form'">
			<thead>
				<tr>
					<th data-options="field:'drmName',width:80">DRM</th>
					<th data-options="field:'spProvince',width:80">省</th>
					<th data-options="field:'spCity',width:80">市</th>
                    <th data-options="field:'belongArea',width:80">区域</th>
					<th data-options="field:'serviceProvidersType',width:80" formatter="formatServiceProvidersType">经销商类型</th>
					<th data-options="field:'dealerName',width:80">经销商名称</th>
	...
	<script type="text/javascript">
		...
		$(function() {
            ...
            //初始化省市区
            getProvince("spProvinceItem");
		});

        // 数据查询并更新表格数据
        function conditionalQuery() {
            var drmName = $("input[name='drmName']").val();
            var spProvince = $("#spProvince").val();
            var spCity = $("#spCity").val();
            var belongArea = $("#belongArea").val();
            var serviceProvidersType = $("#serviceProvidersType").val();
            ...
            searchAndReloadGrid("todoGrid", {
                paramObj:{
                    ...
                    "serviceProvidersType": serviceProvidersType,
        ...
        //列表的 字典值处理
        var serviceProvidersTypeData = [];
		function formatServiceProvidersType(value, row, index){
            var transData = "";
            if(index==0){
                serviceProvidersTypeData = getCodeLibraryToTrans("ServiceProvidersType");
            }
            if(null!=row && null!=row.serviceProvidersType) {
                $.each(serviceProvidersTypeData,
                        function (i, val) {
                            if (row.serviceProvidersType == val.itemNo) {
