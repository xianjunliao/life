...
<@m.htmlTemplate title="待提交申请" javascripts=[] >
	<!-- BODY BEGIN -->
	<@m.navTips content="工作台 &gt; 提单系统 &gt; 业务处理 &gt; 待提交申请"/>
	...
	<div id="area-form" style="width: 100%;">
		...
		<div id="func_operate">
            ...
            <a href="javascript:void(0)" class="easyui-linkbutton" onclick="toSubmit()">提交</a>
            <a href="javascript:void(0)" class="easyui-linkbutton" onclick="toAddSign()">添加签署意见</a>
            <a href="javascript:void(0)" class="easyui-linkbutton" onclick="toCancel()">取消申请</a>
		</div>
	</div>
	<!-- 表单  END-->
		...
		<table id="todoGrid" data-options="toolbar:'#area-form'">
			<thead>
				<tr>
					...
					<th data-options="field:'phaseNo',width:80" hidden="hidden">当前阶段编号</th>
				</tr>
			</thead>
		</table>
	</div>
...
t
		...
		//2.提交
		function toSubmit(domId){
            var rows = $("#todoGrid").datagrid('getSelections');
            if(rows.length > 0) {
                var serialNo = rows[0].serialNo;
                var phaseNo = rows[0].phaseNo;
                parent.$.messager.confirm('提交', '请确认是否提交申请？', function(b) {
                    if (b) {
                        progressLoad();
                        $.ajax({
                            type : "POST",
                            ...
                            url : "${basePath}/bill/contract/submitPendApply/submit",
                            data :JSON.stringify({
                                serialNo: serialNo,
                                phaseNo: phaseNo
                            }),
                            success : function(result) {
                                if (result.code == '200') {
                                    ...
                                    $.messager.alert("提交流程失败，请确认！", result.message,"error");
                                }
                                progressClose();
                            },
                            error:function(data,type, err){
                                $.mytools.errorAlert("系统处理异常，请联系系统管理员！");
                                console.log("提交异常->type:"+ type +", message:"+ err);
